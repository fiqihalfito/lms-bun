...
export async function loader({ request, params, context }: Route.LoaderArgs) {
  const kuisProgress = await PengerjaanKuisService.getKuisProgressById(
    params.idKuisProgress,
  );

  const { toast, headers } = await getToast(request);

  return data({ kuisProgress, toast }, { headers });
}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="p-4 rounded-2xl bg-muted/50 text-center">
              ...
              <p className="text-xl font-bold">
                {kuisProgress?.jumlahBenar ?? 0} /{" "}
                {kuisProgress?.jumlahSoal ?? 0}
              </p>
            </div>
            <div className="p-4 rounded-2xl bg-muted/50 text-center">
              <p className="text-xs text-muted-foreground font-medium uppercase mb-1">
              ...
              <p className="text-xl font-bold">
                {Math.floor(
                  (kuisProgress?.totalWaktuPengerjaanDetik ?? 0) / 60,
                )}
                m {(kuisProgress?.totalWaktuPengerjaanDetik ?? 0) % 60}s
              </p>
            </div>
          </div>
