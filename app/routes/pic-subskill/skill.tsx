...
export async function loader({ request, params, context }: Route.LoaderArgs) {
  ...
  const skills = await SubskillService.getSkillsByIdPicSubSkill(user.idUser);

  return { skills };
}

export default function SkillPicSubSkillRoute({
  loaderData,
  params,
  ...
  return (
    <div>
      <HeaderRoute
        title="Skill"
        description="Skill perlu upload dokumen dan membuat kuis"
      />
              ...
              <Card className="h-full border-l-4 border-l-primary shadow-sm hover:shadow-md">
                <CardHeader className="flex flex-row items-start justify-between space-y-0 pb-2">
                  <div className="space-y-1">
                    <Badge
                      variant="secondary"
                      className="mb-2 w-fit text-xs font-normal"
                    >
                      Skill #{i + 1}
                    </Badge>
