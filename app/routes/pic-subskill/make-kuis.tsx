...
import { ManageKuisService } from "@/features/kuis/services/ManageKuisService";

export async function loader({ request, params, context }: Route.LoaderArgs) {
  // periksa dulu idkuis di msubskill
  // let subskill = await SubskillService.getSubskillByIdWithSkill(
  //   params.idSubSkill,
  // );
  ...
  //
  const { questions, kuisMetaData, idKuis, subskill } =
    await ManageKuisService.makeKuis(params.idSubSkill);

  // toast
  const { headers, toast } = await getToast(request);

  return data(
    { questions, subskill, idKuis, kuisMetaData, toast },
    { headers },
  );
}

export default function MakeKuisRoute({
  loaderData,
  params,
  ...
  return (
    <div className="flex-1 flex flex-col">
      <HeaderRoute
        title={
          <div className="flex items-center gap-2">
            ...
            <Badge className="tracking-wider text-sm">
              {subskill?.skill?.namaSkill}
            </Badge>
      ...
      <div className="mb-6 flex items-center justify-between gap-2 w-full">
        ...
        <SwitchKuisLocked
          idKuis={idKuis}
          isLocked={kuisMetaData.isLocked}
          jumlahSoal={questions.length}
        />
      </div>

      <div className="flex flex-row gap-4 flex-1">
        <div className="border-4 rounded-lg border-dashed flex flex-col p-4 w-full">
          {questions.length === 0 ? (
            <BelumAdaSoal />
          ) : (
                ...
                >
                  ...
                  <ItemContent>
                    ...
                    <ItemDescription>
                      ...
                      Jawaban : {q.answerOption.toUpperCase()} .{" "}
                      {q.correctOption?.optionDesc}
                    </ItemDescription>
                  ...
                  <ItemActions>
                    ...
                    >
                      <NavLink
                        to={`kuis/${idKuis}/question/${q.idKuisQuestion}/edit`}
                        viewTransition
                      >
                        <PencilIcon />
                        Edit
                    ...
                    <Button
                      ...
                      type="button"
                      onClick={() =>
                        fetcherHapusPertanyaan.submit(null, {
                          method: "delete",
