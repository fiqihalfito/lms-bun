...
import { PengerjaanKuisService } from "../services/PengerjaanKuisService";
...
export async function action({ request, params }: Route.ActionArgs) {
  try {
    ...
    // })
    await PengerjaanKuisService.submitCurrentJawaban(
      formData,
    ...
    if (nextQuestionNumber > jumlahSoal) {
      return replace(
        `/app/kuis/${params.idKuis}/progress/${params.idKuisProgress}/submit`,
      );
    } else {
      return replace(
        `/app/kuis/${params.idKuis}/progress/${params.idKuisProgress}/question/${nextQuestionNumber}`,
      );
    }
  } catch (error) {
