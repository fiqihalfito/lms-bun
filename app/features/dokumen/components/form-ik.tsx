...
import type { DefaultValueFormIKSOPProps } from "../lib/types";
import type { loaderLayanan } from "@/features/layanan/lib/types";
import type { loaderGetTeamAll } from "@/features/team/lib/types";
// import { usePresignedUploader } from "../hooks/use-presignedUploader";

type FormIKprop = {
  // mode: "insert" | "update",
  dv?: DefaultValueFormIKSOPProps;
} & React.ComponentProps<typeof Form>;

export function FormIK({ dv, className, ...props }: FormIKprop) {
  const file = useFiles();

  const fetcher = useFetcher({ key: "form-ik" });
  const fetcherSelectLayanan = useFetcher<typeof loaderLayanan>({
    key: "select-layanan",
  });
  const fetcherSelectTeam = useFetcher<typeof loaderGetTeamAll>({
    key: "select-team",
  });
      ...
      <FieldSet>
        ...
        <FieldGroup>
          ...
          <Field>
            ...
            />
            <FieldError id={fields.judul.errorId}>
              {fields.judul.errors}
            </FieldError>
          </Field>
          <Field>
            ...
            <Select
              ...
              label="Layanan"
              items={
                fetcherSelectLayanan.data?.layanan.map((layanan) => ({
              ...
              loadItems={() =>
                fetcherSelectLayanan.load(
                  "/app/resources/layanan/get-layanan-all",
                )
              }
            />
            ...
            <FieldError id={fields.idLayanan.errorId}>
              {fields.idLayanan.errors}
            </FieldError>
          </Field>
          <Field>
            ...
            <Select
              ...
              label="Team"
              items={
                fetcherSelectTeam.data?.teams.map((team) => ({
                  value: team.idTeam,
                  name: team.namaTeam,
                })) ?? []
              }
              ...
              loadItems={() =>
                fetcherSelectTeam.load("/app/resources/team/get-team-all")
              }
            />
            ...
            <FieldError id={fields.idTeam.errorId}>
              {fields.idTeam.errors}
            </FieldError>
          </Field>
          <Field>
            <FieldLabel htmlFor={"upload-dokumen-ik"}>
              upload dokumen
            </FieldLabel>
