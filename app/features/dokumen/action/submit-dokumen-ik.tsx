...
export async function action({ request, params, context }: Route.ActionArgs) {
  ...
  try {
    ...
    if (submission.status !== "success") {
      if (keyFile) {
        await StorageService.deleteFile(keyFile as string);
      }
    ...
    if (!idDokumen) {
      await DokumenService.saveDokumen(
        {
      ...
      return redirectWithSuccess(
        `/app/dokumen/tipe/${params.tipeDokumen}`,
        `Dokumen ${submission.value.judul} berhasil disimpan`,
      );
    } else {
      const oldDokumen = await DokumenService.getDokumenById(
        idDokumen as string,
      );
      if (keyFile) {
        if (oldDokumen[0].filename) {
          await StorageService.deleteFile(oldDokumen[0].filename);
        }
      }
      await DokumenService.editDokumen(idDokumen as string, {
      ...
      return redirectWithSuccess(
        `/app/dokumen/tipe/${params.tipeDokumen}`,
        `Dokumen ${submission.value.judul} berhasil diupdate`,
      );
    }
  } catch (error) {
