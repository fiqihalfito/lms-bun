...
export async function action({ request, params, context }: Route.ActionArgs) {
  ...
  try {
    ...
    if (!idTeam) {
      // insert layanan
      await TeamService.saveTeam(user.idSubBidang!, submission.value.namaTeam);
      return redirectWithSuccess(
        `/app/master/team`,
        `Team ${submission.value.namaTeam} berhasil disimpan`,
      );
    } else {
      // update layanan
      await TeamService.editTeam(idTeam as string, submission.value.namaTeam);
      return redirectWithSuccess(
        `/app/master/team`,
        `Team ${submission.value.namaTeam} berhasil diupdate`,
      );
    }
  } catch (error) {
