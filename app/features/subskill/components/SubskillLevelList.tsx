...
type SubskillLevelListProps = {
  subskills: Awaited<
    ReturnType<typeof SubskillService.getSubskillByIdSkillAndLevel>
  >;
};
    ...
    <ItemGroup className="gap-y-3">
      {subskills.map((subskill, i) => (
        <Item variant="outline" key={i}>
          ...
          <ItemContent>
            <ItemTitle className="font-semibold leading-none">
            ...
            <div className="flex flex-wrap items-center gap-2 mt-2">
              ...
              {subskill.kuisProgress?.idKuisProgress && (
                <Badge
                  ...
                  className="bg-emerald-600 hover:bg-emerald-600"
                >
                  jumlah benar: {subskill.kuisProgress.jumlahBenar}/
                  {subskill.kuisProgress.jumlahSoal}
                </Badge>
              )}
              {subskill.kuisProgress?.idKuisProgress && (
                <Badge
                  ...
                  className={cn(
                    "bg-emerald-600",
                    subskill.kuisProgress.persentaseBenar < 80 && "bg-red-600",
                  )}
                >
                  persentase benar: {subskill.kuisProgress.persentaseBenar}%
                </Badge>
              )}
            </div>
          </ItemContent>
          <ItemActions>
            ...
            {!subskill.dokumen?.statusBacaOne ? (
              ...
              (subskill.kuis?.questions?.length ?? 0) > 0 ? (
              <Button asChild>
