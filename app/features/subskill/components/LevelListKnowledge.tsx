...
import type { SubskillService } from "../services/SubskillService";

type LevelListKnowledge = {
  subskillLevel: Awaited<
    ReturnType<typeof SubskillService.getLevelSubskillListDataByIdSkill>
  >;
};
              ...
              <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                <div className="space-y-1">
                  ...
                  <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-xs font-medium text-muted-foreground">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span>
                        {item.lulusKuis} / {item.jumlahSubskillPerLevel} Lulus
                        Kuis
                      </span>
                    </div>
                    <div className="flex items-center gap-1.5">
                      ...
                      <span>
                        {item.sudahBaca} / {item.jumlahSubskillPerLevel} Sudah
                        Dibaca
                      </span>
                    </div>
                ...
                <div className="flex items-center gap-2 self-start md:self-center">
                  {!item.isUnlocked ? (
                    <Badge
                      variant="outline"
                      className="px-3 py-1 bg-muted/50 text-muted-foreground border-transparent"
                    >
                    ...
                    <Badge
                      variant="outline"
                      className="px-3 py-1 border-primary/50 text-primary bg-primary/5 animate-pulse"
                    >
                      Sedang Berjalan
                    </Badge>
                  )}
        ...
        return (
          <Item
            ...
            className={cn(
              ...
              !item.isUnlocked
                ? "opacity-75 grayscale-[0.3] cursor-not-allowed bg-muted/10"
                : "hover:border-primary/50 hover:shadow-xl hover:shadow-primary/5 translate-y-0 hover:scale-101",
          ...
          >
            {item.isUnlocked ? (
              ...
              <div className="flex items-center w-full gap-6">{content}</div>
            )}
          </Item>
        );
      })}
