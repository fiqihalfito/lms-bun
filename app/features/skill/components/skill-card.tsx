...
import { CheckCircle2Icon, CircleIcon, LogInIcon } from "lucide-react";
import type { SkillStatCardDataItem } from "../lib/types";
import { Button } from "@/components/ui/button";

interface SkillCardProps {
  skill: SkillStatCardDataItem;
  idTeam: string;
}
...
export function SkillCard({ skill, idTeam }: SkillCardProps) {
  ...
  return (
    <Card
      key={skill.idSkill}
      className={cn(
        `flex flex-col h-full transition-all duration-200 hover:shadow-md border bg-card text-card-foreground shadow-sm `,
        isStarted
      ...
      <CardContent className="pt-4 flex-1">
        <div className="grid grid-cols-2 gap-6">
          <div className="space-y-3  ">
            ...
            {skill.groupLevel &&
              Object.entries(skill.groupLevel).map(([levelKey, data]) => {
                  ...
                  <div key={levelKey} className="group">
                    <div className="flex justify-between items-center mb-1.5">
                      <div className="flex items-center gap-2">
                        {data.sudahBaca > 0 ? (
                          <CheckCircle2Icon
                            className={`w-3.5 h-3.5 ${isLevelDone ? "text-green-600" : "text-primary"}`}
                          />
                        ) : (
