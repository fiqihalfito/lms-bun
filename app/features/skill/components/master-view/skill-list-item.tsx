...
import type { SkillDataWithTeamItem } from "../../lib/types";

type SkillListItemProps = {
  item: SkillDataWithTeamItem;
  no: number;
};
  ...
  return (
    <NavLink to={`skill/${idSkill}`}>
      {({ isActive }) => (
        <div
          className={cn(
            "p-4 flex flex-row gap-4 items-center",
            isActive && "bg-primary text-primary-foreground",
          )}
        >
          <div
            className={cn(
              "h-8 w-8 border border-slate-300 rounded text-xs flex items-center justify-center",
