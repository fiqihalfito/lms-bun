...
export abstract class SkillQuery {
  static async findNamaSkillById(idSkill: string) {
  ...
  static async findSkillAndStats(idUser: string, idTeam: string) {
    const t_statBaca = db
      .select({
    ...
    const t_statKuis = db
      .select({
        ...
        persentase:
          sql`${tKuisProgress.totalScore} * 100 / ${tKuisProgress.jumlahSoal}`.as(
            "persentase",
          ),
      })
      .from(tKuisProgress)
    ...
    const t_subskilllevel = db
      .select({
        idSkill: mSubSkill.idSkill,
        level: mSubSkill.level,
        jumlahSubskill: sql`cast(count(${mSubSkill.idSubSkill}) as int)`.as(
          "jumlahSubskill",
        ),
        sudahBaca: sql`cast(count(${t_statBaca.idDokumen}) as int)`.as(
          "sudahBaca",
        ),
        lulusKuis: sql`cast(count(${t_statKuis.idKuis}) as int)`.as(
          "lulusKuis",
        ),
      })
      .from(mSubSkill)
    ...
    const t_skill = await db
      .select({
        ...
        groupLevel: sql<GroupLevelType>`
            ...
            `.as("groupLevel"),
        totalSubskill: sql`sum(${t_subskilllevel.jumlahSubskill})`.mapWith(
          Number,
        ),
        ...
        persentaseProgress:
          sql`((cast(sum(${t_subskilllevel.sudahBaca}) + sum(${t_subskilllevel.lulusKuis}) as float) / 2) * 100) / sum(${t_subskilllevel.jumlahSubskill})`
            .mapWith(Number)
            .as("persentaseProgress"),
      })
      .from(mSkill)
  ...
  static async findSkillsByIdTeam(idTeam: string) {
    const res = await db.query.mSkill.findMany({
      where: {
        idTeam: idTeam,
      },
      extras: {
        jumlahSubskill: (table) =>
          db.$count(mSubSkill, eq(table.idSkill, mSubSkill.idSkill)),
      },
    });
    return res;
  }
