...
export async function action({ request, params, context }: Route.ActionArgs) {
  ...
  try {
    ...
    if (!idLayanan) {
      // insert layanan
      await LayananService.saveLayanan(
        user.idSubBidang!,
        submission.value.namaLayanan,
      );
      return redirectWithSuccess(
        `/app/master/layanan`,
        `Layanan ${submission.value.namaLayanan} berhasil disimpan`,
      );
    } else {
      // update layanan
      await LayananService.editLayanan(
        idLayanan as string,
        submission.value.namaLayanan,
      );
      return redirectWithSuccess(
        `/app/master/layanan`,
        `Layanan ${submission.value.namaLayanan} berhasil diupdate`,
      );
    }
  } catch (error) {
